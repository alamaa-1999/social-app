...
export function ArticleCompose({
    ...
    try {
      ...
      const {postUri, documentUri} = await publishArticle({
        ...
        draft: {
          ...
          description: fresh.subtitle.trim() || fresh.title.trim(),
          markdown,
          flavor,
          facets,
          tags: metadata.tags,
  ...
  const toolbarProps = {
    ...
    onInsertHonorific,
    onRequestBodyFocus: () => {
      // Regains WebView focus without repositioning the cursor - `null`
      // matches TipTap's own "focus without moving the selection"
      // convention. Unlike the old plain-`TextInput` model, there's no
      // caret position to manually restore here: `HonorificBridge`'s own
      // `onBridgeMessage` already does `.chain().focus().insertContent(
      // text).run()` on the web side, in the same transaction as the
      // insert, so the cursor is already exactly where it should be by the
      // time this runs.
